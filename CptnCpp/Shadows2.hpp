...
namespace Gosu
{
	...
	// TODO: Maybe I need more vector operations here..
	struct Vertex
	{
		float x, y, z;
	...
	private:
		...
		long identifier;
		int x, y, maxRange, range, lastModified;
		Gosu::Color color;
		bool dynamic;

	public:
		Light(int x, int y, int maxRange, int range, Gosu::Color color, bool dynamic = false)
			: x(x), y(y), maxRange(maxRange), range(range), color(color), dynamic(dynamic)
		{
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, maxRange, maxRange, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
		...
		int getMaxRange() { return this->maxRange; }
	...
	private:
		...
		static GLuint generateShadowMap(Graphics& graphics, GLuint texture, Gosu::ShadowGeometry &objects, int x, int y, int screenx, int screeny)
		{
			...
			static Gosu::ShaderProgram program = local.compile(Gosu::sharedResourcePrefix() + L"shader/shadows/shadowMap.frag", Gosu::sharedResourcePrefix() + L"shader/shadows/shadowMap.vert");
			glUseProgram(program.program);	

			//std::cout << screenx << " " << screeny << std::endl;
			
			glUniform2f(glGetUniformLocation(program.program, "LightPosition"), (GLfloat)x, (GLfloat)y);
			//glUniform1f(glGetUniformLocation(program.program, "LightRange"), (GLfloat)range);
		...
		static GLuint getShadowMap(Graphics &graphics, Gosu::ShadowGeometry &objects, Gosu::Light &light, int screenx, int screeny)
		{
			...
			return Shadows::generateShadowMap(graphics, light.getTexture(), objects, x, y, screenx, screeny);
/*
			for(std::vector<Gosu::CachedLight>::iterator it = Shadows::cachedLightsRef().begin(); it != Shadows::cachedLightsRef().end(); ++it)
			{
				...
				{
					//std::cout << "found cached shadow map" << endl;
					if((*it).hasChanged(light, light.getX(), light.getY(), screenx, screeny))
					{
						//std::cout << light.getX() << " " << light.getY() << " " << "... and it has changed" << endl;
	...
	public:
		...
		static void drawLight(Graphics &graphics, Light &light, std::vector<Gosu::ShadowGeometry> &objects, int screenx, int screeny)
		{
			...
			Gosu::PostProcessing local(graphics);
			static Gosu::ShaderProgram program = local.compile(Gosu::sharedResourcePrefix() + L"shader/shadows/finalShadow2.frag");
			glUseProgram(program.program);
			...
			glUniform1i(glGetUniformLocation(program.program, "lightmap"), 0);
